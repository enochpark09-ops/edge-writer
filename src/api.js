...
**@떡밥의 내용은 판정하지 않는다.** 떡밥의 투입 회차가 맞는지, '강화' 상태가 정당한지, 장부와 일치하는지는
**도목수 소관**이며 당신에게는 떡밥 장부가 주어지지 않는다. 당신이 가진 확정 타임라인은 회차당 두세 문장짜리
요약이므로, 지나간 회차에 어떤 떡밥이 심겼는지는 거기 남지 않는다. **타임라인에 없다는 이유로 '투입 기록이
없다'고 판정하지 마라.** @떡밥에서 당신이 볼 것은 형식뿐이다 — 줄 모양, 어휘, 빠진 칸.
${COMMON_RULES}
